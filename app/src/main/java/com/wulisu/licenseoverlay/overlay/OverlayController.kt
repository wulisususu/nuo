...
class OverlayController(private val service: AccessibilityService) {
    ...
    private fun readClipboard() {
        ...
        val immediate = clipboardText()
        if (immediate != null) {
            clipboardReading = false
            handleClipboard(immediate)
            return
        }
    ...
    private fun openPanel(initialMessage: String) {
        ...
        val root = LinearLayout(service).apply {
            ...
            background = rounded(0xE61E1E1E.toInt(), dp(16).toFloat())
        }
        val titleRow = LinearLayout(service).apply { orientation = LinearLayout.HORIZONTAL }
        val title = label("激活助手", 16f, true).apply {
        ...
        titleRow.addView(close, LinearLayout.LayoutParams(dp(44), dp(36)))
    ...
    private fun actionButton(text: String, action: LicenseAction): Button = Button(service).apply {
        this.text = text
        isAllCaps = false
        tag = action
    ...
    private fun onAction(action: LicenseAction, button: Button) {
        ...
        setButtonsEnabled(false)
        setStatus("处理中…")
        latestRequestId = api.execute(command) { requestId, result ->
            handler.post {
    ...
    private fun queryCurrent() {
        val code = currentCode ?: return
        setButtonsEnabled(false)
        latestRequestId = api.query(code) { requestId, result ->
            handler.post {
                if (requestId != latestRequestId || currentCode != code) return@post
    ...
    private fun setStatus(text: String) {
        panel?.findViewWithTag<TextView>(TAG_STATUS)?.text = text
    }

    private fun setButtonsEnabled(enabled: Boolean) {
        LicenseAction.entries.forEach { panel?.findViewWithTag<Button>(it)?.isEnabled = enabled }
    }
    ...
    private fun label(text: String, size: Float, bold: Boolean) = TextView(service).apply {
        this.text = text
        textSize = size
        setTextColor(Color.WHITE)
        gravity = Gravity.CENTER_VERTICAL
